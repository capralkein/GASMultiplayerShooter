...
void FAbilitySet_GrantedHandles::TakeFromAbilitySystem(UMPShooterAbilitySystemComponent* ASC)
{
	check(ASC);
...
void UAbilitySet::GiveToAbilitySystem(UMPShooterAbilitySystemComponent* ASC,
	FAbilitySet_GrantedHandles* OutGrantedHandles, UObject* SourceObject) const
{
	...
	for (int32 SetIndex = 0; SetIndex < GrantedAttributes.Num(); ++SetIndex)
	{
		...
		{
			ABILITY_LOG(Error, TEXT("GrantedAttributes[%d] on ability set [%s] is not valid"), SetIndex, *GetNameSafe(this));
			continue;
		}

		UMPShooterAttributeSet* AttributeSet = ASC->GetOrCreateAttributeSet(SetToGrant.AttributeSet);

		check(AttributeSet);
		
		const int32 Level = FMath::RandRange(SetToGrant.RandLevelRange.Min, SetToGrant.RandLevelRange.Max);
		AttributeSet->InitFromInitializers(SetToGrant.Initializers, Level);
	...
	for (int32 AbilityIndex = 0; AbilityIndex < GrantedGameplayAbilities.Num(); ++AbilityIndex)
	{
		...
		UMPShooterGameplayAbility* AbilityCDO = AbilityToGrant.Ability->GetDefaultObject<UMPShooterGameplayAbility>();
